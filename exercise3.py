...
class Cart:
    ...
    def add_item(self, item: dict, qty: int = 1) -> None:
        ...
        if not item["available"]:
            raise OutOfStockError("Item is unavailable")
...
if __name__ == "__main__":
    ...
    try:
        ...
        print(f"Invalid quantity rejected: {e}")

    try:
        cart.add_item(miso)
    except OutOfStockError as e:
        print(f"Unavailable item rejected: {e}")

    try:
        cart.remove_item(gyoza["id"])
    except KeyError as e:
        print(f"Removal rejected: item {e.args[0]} is not in the cart")
